...
bool writeMeshFile(const Mesh &mesh, std::string path)
{
    ...
    char attributes =
        (char)MeshAttributes::VERTICES | (char)MeshAttributes::NORMALS;
...
void createMeshVao(Mesh &mesh)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, mesh.num_vertices * sizeof(glm::fvec3),
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                 mesh.num_indices * sizeof(unsigned short), &mesh.indices[0],
                 GL_STATIC_DRAW);
...
bool readMeshFile(Mesh &mesh, std::string path)
{
    ...
    mesh.indices =
        (unsigned short *)malloc(sizeof(unsigned short) * mesh.num_indices);
    ...
    mesh.vertices =
        (glm::fvec3 *)malloc(sizeof(glm::fvec3) * mesh.num_vertices);
...
// Just here for reference at this point since ASSIMP can't actually be compiled with this project
// In future I may create a standalone tool for generating my mesh files @todo
#ifdef USE_ASSIMP
...
bool loadMeshWithAssimp(Mesh &mesh, std::string path)
{
    ...
    for (int i = 0; i < scene->mNumMeshes; ++i)
    {
        ...
        printf("Loading mesh index %d from face %d ---> %d.\n", i,
               mesh.draw_start[i], mesh.draw_start[i] + mesh.draw_count[i] - 1);
    }
    ...
    for (int j = 0; j < scene->mNumMeshes; ++j)
    {
        ...
        {
            mesh.vertices[vertices_offset + i] =
                glm::fvec3(ai_mesh->mVertices[i].x, ai_mesh->mVertices[i].y,
        ...
        if (ai_mesh->mNormals != NULL)
        {
            for (unsigned int i = 0; i < ai_mesh->mNumVertices; i++)
            {
                mesh.normals[vertices_offset + i] =
                    glm::fvec3(ai_mesh->mNormals[i].x, ai_mesh->mNormals[i].y,
        ...
        for (unsigned int i = 0; i < ai_mesh->mNumFaces; i++)
        {
            ...
            mesh.indices[indices_offset + 3 * i] =
                ai_mesh->mFaces[i].mIndices[0];
            mesh.indices[indices_offset + 3 * i + 1] =
                ai_mesh->mFaces[i].mIndices[1];
            mesh.indices[indices_offset + 3 * i + 2] =
